...
public:
    ...
    Vec3 vtransform(const Vec3 &vec) const {
        const Mat4x4 &me = *this;
        return Vec3(
            me(0, 0) * vec.x + me(0, 1) * vec.y + me(0, 2) * vec.z,
            me(1, 0) * vec.x + me(1, 1) * vec.y + me(1, 2) * vec.z,
            me(2, 0) * vec.x + me(2, 1) * vec.y + me(2, 2) * vec.z);
    }
